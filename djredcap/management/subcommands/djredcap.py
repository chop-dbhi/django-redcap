...
def csv_2_json(self, reader, file_name):
    ...
    fout = open(os.path.join(os.path.dirname(file_name), new_file_name + '.json'), "w+")
    ...
    for row in reader:
        ...
        if row['field_name'].find('startrepeat') != -1:
            ...
            form_name = make_singular(self, form_2_model(self, '_'.join(repeat_info[3:])))
    ...
    for name in form_names_list:
        if name == form_name:
            ...
            if end_digit:
                ...
                form_name[end_digit:] = str(int("".join(form_name[end_digit:]))+1)
